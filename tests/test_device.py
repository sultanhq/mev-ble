...
@pytest.mark.asyncio
async def test_pair_retrieves_generated_setup_code() -> None:
    """Physical pairing requests and stores the code generated by the unit."""

    # Arrange - return a nonzero little-endian code after the zero request.
    client = DeviceClient([])
    original_read = client.read_gatt_char
    ...
    # Assert - zero requested a code and the little-endian value was returned.
    assert (
        PIN_CHARACTERISTIC_UUID,
        bytes(4),
        True,
...
@pytest.mark.asyncio
async def test_pair_rejects_zero_setup_code(monkeypatch) -> None:
    """Pairing mode that never supplies a code raises a specific error."""

    # Arrange - keep returning zero and remove the retry delay from the test.
    client = DeviceClient([])
    original_read = client.read_gatt_char
    ...
    client.read_gatt_char = read
    monkeypatch.setattr(
        "custom_components.ventaxia_multihome.device.PAIRING_RETRY_INTERVAL", 0
    )
    ...
    # Act / Assert - no generated code is reported as pairing-mode failure.
    with pytest.raises(SetupCodeRejectedError):
